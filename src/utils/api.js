...
const generatePrompt = (queries, labels, examples = []) => {
  // Start with system instructions and labels (this part will be cached)
...
const parseResponse = (content) => {
  try {
    ...
    // Ensure the content is wrapped in array brackets
...
const readCSVFile = (file) => {
  return new Promise((resolve, reject) => {
    Papa.parse(file, {
      complete: (results) => {
        // Filter out empty rows and handle multiple columns if needed
        const data = results.data
          .filter(row => row[0] && row[0].trim());
        
        // If file has multiple columns, assume it's an examples file
